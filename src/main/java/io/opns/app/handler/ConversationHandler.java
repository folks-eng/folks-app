...
import io.opns.app.bo.ConversationBO;
import io.opns.app.model.Conversation;
...
public class ConversationHandler extends AbstractHandler {
    ...
    private final ConversationBO conversationBO;
    
    public ConversationHandler(Vertx vertx) {
        super(vertx);
        this.conversationBO = new ConversationBO();
    }
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            Conversation conversation = MapperUtil.decode(ctx.body().buffer().getBytes(), Conversation.class);
            conversation = conversationBO.create(user(ctx), conversation);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<Conversation> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<Conversation>>() {});
            conversationBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            Conversation conversation = MapperUtil.decode(ctx.body().buffer().getBytes(), Conversation.class);
            conversation.setConversationId(Long.valueOf(id));


            // First fetch the entry, to see if this already exists.
            Conversation rs = conversationBO.modify(user(ctx), conversation);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("Conversation modified successfully");
    ...
    public void view(RoutingContext ctx) {
        ...
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            Conversation conversation = conversationBO.view(user(ctx), Long.valueOf(id));
    ...
    public void viewAll(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            List<Conversation> conversations = conversationBO.viewAll(user(ctx), params);
            List<Object> rows = (List)conversations;
    ...
    public void remove(RoutingContext ctx) {
        ...
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            Conversation conversation = conversationBO.remove(user(ctx), Long.valueOf(id));


            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_NO_CONTENT);
            msg.setMessage("Conversation deleted successfully");
    ...
    private ItemList build(QueryParams params, List<Object> list) {
        ...
        // Build the item list.
        String path = "/api/v1/conversations";
