...
import io.opns.app.bo.CategoryBO;
import io.opns.app.model.Category;
...
public class CategoryHandler extends AbstractHandler {
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            category = categoryBO.create(user(ctx), category);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<Category> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<Category>>() {});
            categoryBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            Category rs = categoryBO.modify(user(ctx), category);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("Category modified successfully");
