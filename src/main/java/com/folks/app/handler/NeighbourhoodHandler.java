...
import com.folks.app.bo.NeighbourhoodBO;
 ...
 */
public class NeighbourhoodHandler extends AbstractHandler {
    
    private final NeighbourhoodBO neighbourhoodBO;
    
    public NeighbourhoodHandler(Vertx vertx) {
        super(vertx);
        this.neighbourhoodBO = new NeighbourhoodBO();
    }
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            Neighbourhood neighbourhood = MapperUtil.decode(ctx.body().buffer().getBytes(), Neighbourhood.class);
            neighbourhood = neighbourhoodBO.create(user(ctx), neighbourhood);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<Neighbourhood> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<Neighbourhood>>() {});
            neighbourhoodBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            Neighbourhood neighbourhood = MapperUtil.decode(ctx.body().buffer().getBytes(), Neighbourhood.class);
            neighbourhood.setNeighbourhoodId(Integer.valueOf(id));


            // First fetch the entry, to see if this already exists.
            Neighbourhood rs = neighbourhoodBO.modify(user(ctx), neighbourhood);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("Neighbourhood modified successfully");
    ...
    public void view(RoutingContext ctx) {
        ...
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            Neighbourhood neighbourhood = neighbourhoodBO.view(user(ctx), Integer.valueOf(id));
    ...
    public void viewAll(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            List<Neighbourhood> neighbourhoods = neighbourhoodBO.viewAll(user(ctx), params);
            List<Object> rows = (List)neighbourhoods;
    ...
    public void remove(RoutingContext ctx) {
        ...
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            Neighbourhood neighbourhood = neighbourhoodBO.remove(user(ctx), Integer.valueOf(id));

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_NO_CONTENT);
            msg.setMessage("Neighbourhood deleted successfully");
