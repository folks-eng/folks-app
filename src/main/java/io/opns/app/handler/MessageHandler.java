...
import io.opns.app.bo.MessageBO;
import io.opns.app.model.Message;
...
public class MessageHandler extends AbstractHandler {
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            Message message = MapperUtil.decode(ctx.body().buffer().getBytes(), Message.class);
            message = messageBO.create(user(ctx), message);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<Message> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<Message>>() {});
            messageBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            Message message = MapperUtil.decode(ctx.body().buffer().getBytes(), Message.class);
            message.setMessageId(Long.valueOf(id));


            // First fetch the entry, to see if this already exists.
            Message rs = messageBO.modify(user(ctx), message);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("Message modified successfully");
    ...
    public void view(RoutingContext ctx) {
        ...
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            Message message = messageBO.view(user(ctx), Long.valueOf(id));
    ...
    public void viewAll(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            List<Message> messages = messageBO.viewAll(user(ctx), params);
            List<Object> rows = (List)messages;
    ...
    public void remove(RoutingContext ctx) {
        ...
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            Message message = messageBO.remove(user(ctx), Long.valueOf(id));


            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_NO_CONTENT);
            msg.setMessage("Message deleted successfully");
