...
public final class AvailSlotUtil {
    ...
    public static String buildAvailabilityQuery(Integer serviceId, Short durationMin, Date date) {
        ...
        buff.append("WITH avail_status AS (").append("\n")
                .append("    SELECT c.professional_id").append("\n")
                .append("           , c.date").append("\n")
                .append("           , c.start_time AS slot_1").append("\n");

        for (short i = 0; i < slots; i++) {
            buff.append("           , CASE LEAD(c.start_time, ").append(i).append(") OVER w").append("\n")
                    .append("                WHEN '17:00:00'::time THEN '18:00:00'::time").append("\n")
                    .append("                ELSE LEAD(c.start_time, ").append(i + 1).append(") OVER w").append("\n")
        ...
        buff.append("      FROM fks_services a").append("\n")
                ...
                .append("     INNER JOIN fks_availabilities c ON (b.professional_id = c.professional_id AND c.date = ? AND c.is_booked = 0)").append("\n")
                .append("     WHERE a.service_id = ?").append("\n");

        buff.append("    WINDOW w AS (").append("\n")
                .append("        PARTITION BY c.professional_id, c.date").append("\n")
                .append("        ORDER BY c.start_time").append("\n")
                .append("    )").append("\n")
                .append(")").append("\n");
        ...
        String fairQuery = """
            ...
            prof_eligibility AS (
                SELECT b.professional_id, b.date, p.work_start, p.work_end, MIN(b.start_time) AS start_time, MAX(b.end_time) AS end_time, COUNT(*) AS slot_count
                 ...
                 INNER JOIN fks_availabilities b ON (a.professional_id = b.professional_id AND a.service_id = ?)
                 CROSS JOIN params p
