...
import io.opns.app.bo.PricingRuleBO;
import io.opns.app.model.PricingRule;
 ...
 */
public class PricingRuleHandler extends AbstractHandler {
    
    private final PricingRuleBO pricingRuleBO;
    
    public PricingRuleHandler(Vertx vertx) {
        super(vertx);
        this.pricingRuleBO = new PricingRuleBO();
    }
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            pricingRule = pricingRuleBO.create(user(ctx), pricingRule);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<PricingRule> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<PricingRule>>() {});
            pricingRuleBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            PricingRule rs = pricingRuleBO.modify(user(ctx), pricingRule);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("PricingRule modified successfully");
    ...
    public void viewAll(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            List<PricingRule> pricingRules = pricingRuleBO.viewAll(user(ctx), params);
            List<Object> rows = (List)pricingRules;
    ...
    public void remove(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            msg.setMessage("PricingRule deleted successfully");
    ...
    private ItemList build(QueryParams params, List<Object> list) {
        ...
        // Build the item list.
        String path = "/api/v1/pricingRules";
