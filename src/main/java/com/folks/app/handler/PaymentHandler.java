...
import com.folks.app.bo.PaymentBO;
...
public class PaymentHandler extends AbstractHandler {
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            payment = paymentBO.create(user(ctx), payment);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<Payment> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<Payment>>() {});
            paymentBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            Payment rs = paymentBO.modify(user(ctx), payment);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("Payment modified successfully");
    ...
    public void viewAll(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            List<Payment> payments = paymentBO.viewAll(user(ctx), params);
            List<Object> rows = (List)payments;
    ...
    public void remove(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            msg.setMessage("Payment deleted successfully");
