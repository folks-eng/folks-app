...
import com.folks.app.bo.DocumentBO;
...
public class DocumentHandler extends AbstractHandler {
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            document = documentBO.create(user(ctx), document);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<Document> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<Document>>() {});
            documentBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            Document rs = documentBO.modify(user(ctx), document);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("Document modified successfully");
    ...
    public void viewAll(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            List<Document> documents = documentBO.viewAll(user(ctx), params);
            List<Object> rows = (List)documents;
    ...
    public void remove(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            msg.setMessage("Document deleted successfully");
