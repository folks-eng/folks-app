...
import com.folks.app.bo.WalletBO;
...
public class WalletHandler extends AbstractHandler {
    ...
    private final WalletBO walletBO;
    
    public WalletHandler(Vertx vertx) {
        super(vertx);
        this.walletBO = new WalletBO();
    }
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            Wallet wallet = MapperUtil.decode(ctx.body().buffer().getBytes(), Wallet.class);
            wallet = walletBO.create(user(ctx), wallet);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<Wallet> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<Wallet>>() {});
            walletBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            Wallet wallet = MapperUtil.decode(ctx.body().buffer().getBytes(), Wallet.class);
            wallet.setWalletId(Long.valueOf(id));


            // First fetch the entry, to see if this already exists.
            Wallet rs = walletBO.modify(user(ctx), wallet);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("Wallet modified successfully");
    ...
    public void view(RoutingContext ctx) {
        ...
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            Wallet wallet = walletBO.view(user(ctx), Long.valueOf(id));
    ...
    public void viewAll(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            List<Wallet> wallets = walletBO.viewAll(user(ctx), params);
            List<Object> rows = (List)wallets;
    ...
    public void remove(RoutingContext ctx) {
        ...
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            Wallet wallet = walletBO.remove(user(ctx), Long.valueOf(id));


            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_NO_CONTENT);
            msg.setMessage("Wallet deleted successfully");
