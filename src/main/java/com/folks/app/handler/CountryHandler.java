...
import com.folks.app.bo.CountryBO;
...
public class CountryHandler extends AbstractHandler {
    ...
    public CountryHandler(Vertx vertx) {
        super(vertx);
        this.countryBO = new CountryBO();
    }
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            Country country = MapperUtil.decode(ctx.body().buffer().getBytes(), Country.class);
            country = countryBO.create(user(ctx), country);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<Country> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<Country>>() {});
            countryBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            Country rs = countryBO.modify(user(ctx), country);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("Country modified successfully");
    ...
    public void view(RoutingContext ctx) {
        ...
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            Country country = countryBO.view(user(ctx), Integer.valueOf(id));
    ...
    public void viewAll(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            List<Country> countrys = countryBO.viewAll(user(ctx), params);
            List<Object> rows = (List)countrys;
    ...
    public void remove(RoutingContext ctx) {
        ...
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            Country country = countryBO.remove(user(ctx), Integer.valueOf(id));

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_NO_CONTENT);
            msg.setMessage("Country deleted successfully");
