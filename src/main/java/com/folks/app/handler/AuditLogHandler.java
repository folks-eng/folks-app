...
import com.folks.app.bo.AuditLogBO;
...
public class AuditLogHandler extends AbstractHandler {
    ...
    private final AuditLogBO auditLogBO;
    
    public AuditLogHandler(Vertx vertx) {
        super(vertx);
        this.auditLogBO = new AuditLogBO();
    }
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            auditLog = auditLogBO.create(user(ctx), auditLog);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<AuditLog> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<AuditLog>>() {});
            auditLogBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            AuditLog rs = auditLogBO.modify(user(ctx), auditLog);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("AuditLog modified successfully");
    ...
    public void viewAll(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            List<AuditLog> auditLogs = auditLogBO.viewAll(user(ctx), params);
            List<Object> rows = (List)auditLogs;
    ...
    public void remove(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            msg.setMessage("AuditLog deleted successfully");
