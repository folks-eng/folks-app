...
public class PricingRuleDAOImpl implements PricingRuleDAO {
    ...
    private final String TABLE = "fks_pricing_rules";
    
    @PersistenceContext(name = "folks-app-pu")
    private EntityManager em;
    
    @Override
    public void insert(PricingRule record) {
        insert(Arrays.asList(record));
    }

    @Override
    public void insert(List<PricingRule> records) {
        for (PricingRule record : records) {
            em.persist(record);
        }
    }

    @Override
    public void update(PricingRule record) {
        update(Arrays.asList(record));
    }

    @Override
    public void update(List<PricingRule> records) {
        for (PricingRule record : records) {
            em.merge(record);
        }
    }
    ...
    @Override
    public void delete(PricingRule record) {
        em.remove(record);
    }

    @Override
    public PricingRule find(PricingRule.PricingRulePK pk) {
        return em.find(PricingRule.class, pk);
    }

    @Override
    public List<PricingRule> query(SearchCriteria search) {
        ...
        TypedQuery q = em.createNativeQuery(query.toQuery(), PricingRule.class);
        ...
        List<PricingRule> result = q.getResultList();
        return result;
    }
