...
import com.folks.app.bo.CouponBO;
...
public class CouponHandler extends AbstractHandler {
    ...
    private final CouponBO couponBO;
    
    public CouponHandler(Vertx vertx) {
        super(vertx);
        this.couponBO = new CouponBO();
    }
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            Coupon coupon = MapperUtil.decode(ctx.body().buffer().getBytes(), Coupon.class);
            coupon = couponBO.create(user(ctx), coupon);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<Coupon> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<Coupon>>() {});
            couponBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            Coupon coupon = MapperUtil.decode(ctx.body().buffer().getBytes(), Coupon.class);
            coupon.setCouponId(Integer.valueOf(id));


            // First fetch the entry, to see if this already exists.
            Coupon rs = couponBO.modify(user(ctx), coupon);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("Coupon modified successfully");
    ...
    public void view(RoutingContext ctx) {
        ...
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            Coupon coupon = couponBO.view(user(ctx), Integer.valueOf(id));
    ...
    public void viewAll(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            List<Coupon> coupons = couponBO.viewAll(user(ctx), params);
            List<Object> rows = (List)coupons;
    ...
    public void remove(RoutingContext ctx) {
        ...
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            Coupon coupon = couponBO.remove(user(ctx), Integer.valueOf(id));


            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_NO_CONTENT);
            msg.setMessage("Coupon deleted successfully");
