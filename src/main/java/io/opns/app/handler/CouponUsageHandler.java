...
import io.opns.app.bo.CouponUsageBO;
import io.opns.app.model.CouponUsage;
 ...
 */
public class CouponUsageHandler extends AbstractHandler {
    
    private final CouponUsageBO couponUsageBO;
    
    public CouponUsageHandler(Vertx vertx) {
        super(vertx);
        this.couponUsageBO = new CouponUsageBO();
    }
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            couponUsage = couponUsageBO.create(user(ctx), couponUsage);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<CouponUsage> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<CouponUsage>>() {});
            couponUsageBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            CouponUsage rs = couponUsageBO.modify(user(ctx), couponUsage);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("CouponUsage modified successfully");
    ...
    public void viewAll(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            List<CouponUsage> couponUsages = couponUsageBO.viewAll(user(ctx), params);
            List<Object> rows = (List)couponUsages;
    ...
    public void remove(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            msg.setMessage("CouponUsage deleted successfully");
    ...
    private ItemList build(QueryParams params, List<Object> list) {
        ...
        // Build the item list.
        String path = "/api/v1/couponUsages";
