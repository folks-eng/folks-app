...
import com.folks.app.bo.JobStatusBO;
 ...
 */
public class JobStatusHandler extends AbstractHandler {
    
    private final JobStatusBO jobStatusBO;
    
    public JobStatusHandler(Vertx vertx) {
        super(vertx);
        this.jobStatusBO = new JobStatusBO();
    }
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            jobStatus = jobStatusBO.create(user(ctx), jobStatus);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<JobStatus> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<JobStatus>>() {});
            jobStatusBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            JobStatus rs = jobStatusBO.modify(user(ctx), jobStatus);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("JobStatus modified successfully");
    ...
    public void remove(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            msg.setMessage("JobStatus deleted successfully");
