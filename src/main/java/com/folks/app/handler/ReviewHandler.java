...
import com.folks.app.bo.ReviewBO;
...
public class ReviewHandler extends AbstractHandler {
    ...
    public ReviewHandler(Vertx vertx) {
        super(vertx);
        this.reviewBO = new ReviewBO();
    }
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            review = reviewBO.create(user(ctx), review);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<Review> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<Review>>() {});
            reviewBO.create(user(ctx), list);
    ...
    public void modify(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            Review rs = reviewBO.modify(user(ctx), review);

            ServerMessage msg = new ServerMessage();
            msg.setCode(HttpURLConnection.HTTP_OK);
            msg.setMessage("Review modified successfully");
    ...
    public void viewAll(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            List<Review> reviews = reviewBO.viewAll(user(ctx), params);
            List<Object> rows = (List)reviews;
    ...
    public void remove(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            msg.setMessage("Review deleted successfully");
