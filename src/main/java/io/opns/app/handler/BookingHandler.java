...
import io.opns.app.bo.BookingBO;
import io.opns.app.model.Booking;
...
public class BookingHandler extends AbstractHandler {
    ...
    public BookingHandler(Vertx vertx) {
        super(vertx);
    ...
    public void create(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            ...
            booking = bookingBO.create(user(ctx), booking);
    ...
    public void batchCreate(RoutingContext ctx) {
        // If you use a remote store, this method will safely execute the blocking code.
        vertx().executeBlocking(() -> {
            List<Booking> list = MapperUtil.mapper().readValue(ctx.body().buffer().getBytes(), new TypeReference<List<Booking>>() {});
            bookingBO.create(user(ctx), list);
    ...
    public void viewAll(RoutingContext ctx) {
        ...
        vertx().executeBlocking(() -> {
            List<Booking> bookings = bookingBO.viewAll(user(ctx), params);
            List<Object> rows = (List)bookings;
