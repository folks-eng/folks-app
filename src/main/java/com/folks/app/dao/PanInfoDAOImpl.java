...
import com.folks.app.model.PanInfo;
...
public class PanInfoDAOImpl implements PanInfoDAO {
    ...
    private final String TABLE = "pan_info";
    
    @PersistenceContext(name = "folks-app-pu")
    private EntityManager em;
    
    @Override
    public void insert(PanInfo record) {
        insert(Arrays.asList(record));
    }

    @Override
    public void insert(List<PanInfo> records) {
        for (PanInfo record : records) {
            em.persist(record);
        }
    }

    @Override
    public void update(PanInfo record) {
        update(Arrays.asList(record));
    }

    @Override
    public void update(List<PanInfo> records) {
        for (PanInfo record : records) {
            em.merge(record);
        }
    }
    ...
    @Override
    public void delete(PanInfo record) {
        em.remove(record);
    }

    @Override
    public PanInfo find(PanInfo.PanInfoPK pk) {
        return em.find(PanInfo.class, pk);
    }

    @Override
    public List<PanInfo> query(SearchCriteria search) {
        ...
        TypedQuery q = em.createNativeQuery(query.toQuery(), PanInfo.class);
        ...
        List<PanInfo> result = q.getResultList();
        return result;
    }
